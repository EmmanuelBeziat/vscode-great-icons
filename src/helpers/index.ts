...
export const getThemeConfig = (section: string) => {
	return getConfig('vscode-great-icons').inspect(section)
}
...
export const setThemeConfig = (section: string, value: any, global: boolean = false) => {
	return getConfig('vscode-great-icons').update(section, value, global)
}
...
export const isThemeConfigured = (global: boolean = false): boolean => {
	return global ? getConfig().inspect('workbench.iconTheme').globalValue === 'vscode-great-icons'
		: getConfig().inspect('workbench.iconTheme').workspaceValue === 'vscode-great-icons'
}
...
export const getCurrentExtensionVersion = (): string => {
	return vscode.extensions.getExtension('emmanuelbeziat.vscode-great-icons').packageJSON.version
}
...
export const getSettingsFilePath = (): string => {
	...
	return path.join(codeUserDataPath, 'vscode-great-icons.json')
}
...
export const getMaterialIconsJSON = (): Promise<IconConfiguration> => {
	return new Promise((resolve, reject) => {
		const iconJSONPath = path.join(getExtensionPath(), 'app', 'src', 'icons.json')
